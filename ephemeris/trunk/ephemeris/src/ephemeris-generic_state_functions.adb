...
package body Ephemeris.Generic_State_Functions is
   ...
   procedure Interpolate(T        : Real; -- relative time (0.0..1.0)
                         ...
                         X        : out State)
   is
      I     : Natural := Object'Pos(Target) + 1; -- target object index (1..11)
      Index : Natural;                        -- index to coefficients
      I0    : Natural := Pointers(1,I);       -- beginning of coefficients
      NC    : Natural := Pointers(2,I);       -- no. of coefficients
      NG    : Natural := Pointers(3,I);       -- no. of granules
      NM    : Natural := 3;                   -- no. of components
   ...
   begin
      ...
      G  := Natural(Real'Truncation((Real(NG)*T - Real'Truncation(T)))) + 1;
      TC :=
        2.0*((Real(NG)*T - Real'Truncation(Real(NG)*T)) + Real'Truncation(T))
        - 1.0;
