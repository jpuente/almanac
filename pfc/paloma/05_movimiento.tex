...
\begin{figure}[hbt]
  % Requires \usepackage{graphicx}
  \centering\includegraphics[scale=0.5]{SmoothT}\\
  \vspace{0.2cm}
...
A través de un valor de $d$ corregido al tenerse en cuenta el radio del robot, se guardan para cada punto de la trayectoria las dos distancias mínimas a algún obstáculo a cada uno de los dos lados de la misma. Estas distancias se llamarán \prog{min\_int[i]} y \prog{max\_int[i]}, respectivamente. Sus valores están inicializados con la máxima desviación que se le permite al robot, para el caso en que no haya ningún obstáculo en alguno de los lados y otras situaciones similares. Si después de recalcular dichos valores se cumple que \prog{min\_int > max\_int}, el robot no puede evitar los obstáculos sin superar la máxima desviación establecida y se dice que se encuentra en estado de bloqueo.

Haciendo la media entre esos dos valores se obtiene el desplazamiento, \prog{error [i]}, que debe aplicarse sobre el punto para que quede a la misma distancia de cada uno de los límites hallados para ambos lados (figura \ref{fg:elasticTray}).
...
\noindent
Se guardan los argumentos en variables miembro, para que sus valores sean utilizados por el control de movimiento, y se establece el estado como \prog{NO_} \prog{LLEGADO} mediante una macro con este nombre.
...
Esta función es la implementación del algoritmo descrito en el apartado \ref{tray}. Se devuelve el vector \prog{smooth_tray}, que contiene los puntos de la trayectoria suave obtenida.
...
siendo $dist$ la longitud del segmento inicial. Finalmente se iguala el vector \prog{nom_tray} al vector con los nuevos puntos (figura \ref{fg:divide}).
...
\begin{figure}[h]
  % Requires \usepackage{graphicx}
  \centering\includegraphics[scale=0.5]{obs2}\\
    \caption{Experimento previo para el control reactivo (a)}\label{fg:react1a}
  \end{figure}

\begin{figure}[h]
  \centering\includegraphics[scale=0.5]{obs3}
...
\begin{figure}[h]
  % Requires \usepackage{graphicx}
  \centering\includegraphics[scale=0.6]{obs4}\\
...
\begin{figure}[h]
  % Requires \usepackage{graphicx}
  \centering\includegraphics[scale=0.8]{react2}\\
...
\begin{figure}[h]
  \centering\includegraphics[scale=0.5]{real}
  \caption{Primer experimento de movimiento con robot real (a)}\label{fg:real1a}
\end{figure}

\begin{figure}[h]
  \centering\includegraphics[scale=1.1]{real1}
